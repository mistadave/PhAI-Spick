...
$
$
  Delta m &= abs(-F_H/(g^2 dot sin(alpha)) dot colmagenta(Delta g)) + abs(-(F_H dot cos(alpha))/(g dot sin^2 (F_H ) ) dot colorange(Delta alpha)) \ &+ abs(
    1/(g dot sin(alpha)) dot colgreen(
      Delta
      F_H
    )
  ) =0.191"kg"
